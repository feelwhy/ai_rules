...
Each model class lives in its own file under `models/`. A new `_name` and an `_inherit` of another model are two files, even when the inherit only adds one field. Name the file after the model (`babylon_rate_bracket.py`, `res_company.py`). Import each file from `models/__init__.py`.

Never put two `models.Model` or `models.TransientModel` classes in one file.
...
Image / Binary fields (except `ir.attachment.raw`) **reject `bytes`**:
`TypeError: …image_1920: use BinaryValue instead of bytes`. 19.0
`base64.b64encode(fh.read())` is `bytes`, so it dies. A base64 **str** still
works (RPC path). Native write is `BinaryBytes(raw_jpeg)` from
`odoo.tools.binary`. Observed `20_16` Gx.5
`_demo_ba_stamp_car_rent_variants`. Checker `python-api` greps
`base64.b64encode` assigned into a field.

```python
# BEFORE
vals["image_1920"] = base64.b64encode(fh.read())
# AFTER
from odoo.tools.binary import BinaryBytes
vals["image_1920"] = BinaryBytes(fh.read())
```
...
### `archInfo.openAction` left the KanbanRecord — use `this.props.openAction`

19.0 `KanbanRecord.onGlobalClick` read `archInfo.openAction` from
`this.props` (`kanban_record.js:291`). saas-19.4 moved that onto a
**prop**: the renderer writes
`openAction="this.props.archInfo.openAction"`
(`kanban_renderer.xml:53`, `:87`) and the record reads
`const { openAction } = this.props` (`:237`). `archInfo` is no
longer passed. A leftover `archInfo.openAction.action` is
`undefined.action`.

Observed `20_4` Gx.8: opening a Password Manager bundle died
`TypeError: Cannot read properties of undefined (reading 'action')`
at `BundleKanbanRecord.onGlobalClick`. Successor: destructure
`openAction` from `this.props` like core, keep the module
`buttonContext` on `doActionButton`, fall through to `openRecord`
when there is no action. Checker `js-symbol` greps
`archInfo.openAction`.

### Dialog Save left `web.FormView.Buttons` — use `buttonDialogTemplate`

19.0 `web.FormView.Buttons` was Save / Discard / Remove / New
(`form_controller.xml`). saas-19.4 split that: `web.FormView.Buttons`
is only New, and dialogs `t-call` `this.props.buttonDialogTemplate`
(`web.FormView.DialogButtons`, still Save). A leftover inherit of
`FormView.Buttons` that xpaths `o_form_button_save` never matches, so
the dialog keeps Save. Setting only `buttonTemplate` is ignored inside
a dialog.

Observed `20_4` Gx.8: Password Manager login showed Save / Discard and
Enter did nothing. Successor: inherit `web.FormView.DialogButtons`,
pass `buttonDialogTemplate` on the `View` props (same template name
is fine). Checker `owl-xpath` now indexes `contains(@class, …)` the
...
**both** refs (`fromUnityToServerValues`). `value[0]` is `undefined`.
19.0 `browse` often swallowed that; saas-19.4 asserts
`Invalid falsy real id` (`odoo/orm/models.py` `browse`).

Observed `20_4` Gx.8: login-dialog eye / copy called
`password.bundle.action_check_bundle_key` with `[undefined]` because
`bundle_id[0]` was gone. Successor: `value.id` (keep a `[id, name]`
fallback if the value can still be an array). Guard `orm.call` id
lists the same way as the `20_5` formula `browse([False])`. Checker
`js-symbol` greps `.\w+_id[0]`.

### `static props = { ...CharField.props }` does not keep extras

19.0 `CharField` declares `static props`. saas-19.4 moved the schema to
an **instance** `props = props(charFieldProps)` (`char_field.js`). That
function **strips** keys that are not in the schema. A leftover

```js
static props = { ...CharField.props, logIn: { type: Boolean, optional: true } };
```

does nothing: `CharField.props` is undefined, and the inherited instance
`props()` still drops `logIn`. Reading `activeFields.attrs` is not enough
when the widget already treated the field as a normal password.

Observed `20_4` Gx.8: after DialogButtons, login still showed regenerate
and Enter did not submit. Successor matches enterprise
`AccountAuditClickableCharField`:

```js
import { props, t } from "@odoo/owl";
import { CharField, charFieldProps } from "@web/views/fields/char/char_field";
props = props({ ...charFieldProps, logIn: t.boolean().optional() });
```
...
`DynamicList.orderBy` is `this.config.orderBy` at both refs. saas
`sortBy(fieldName)` (`dynamic_list.js:190`) toggles: same first term
with `asc=false` becomes `[]`, and empty falls back to
`defaultOrderBy`. PWM `_applySorting` wrote `{name, asc: false}` then
`sortBy(this.kanbanOrder)` — changing Sort by did nothing.

Observed `20_4` Gx.8: All Passwords Sort by. Other ported groups have
no `.sortBy(`. File Manager already `_load`s. Successor:

```js
await this.props.kanbanList.load({
    orderBy: [{ name: field, asc }, { name: "id", asc: true }],
});
```

Checker `js-symbol` greps `config.orderBy = []`.

### `getRecordClasses` is gone — use `getCardClasses`

19.0 `KanbanRecord` was a standalone `<article t-att-class="getRecordClasses()">`.
saas-19.4 `KanbanRecord` extends `CardRenderer`; the article class is
`this.getCardClasses()` (`kanban_record.js:120`, `card_renderer.js:128`).
`getRecordClasses` is deleted. A leftover override never runs, so
`jstr-kanban-selected` never lands and the top-right checkbox stays
empty (PWM / File Manager). KPI already used `getCardClasses`.
...
Checker `js-symbol` greps `getRecordClasses(`.
...
`get_str(key, default)` does **not** use `default` when the key exists as `""`.
`_get` returns None / `INVALID_VALUE` only (`ir_config_parameter.py:124-129` at the pin);
a stored empty string is returned as-is. Settings inverses write `""` when the
m2m is empty. `safe_eval("")` is `SyntaxError`. Observed `20_4` Gx.8 portal vault
after login (`odoo_password_manager.pw_custom_portal_fields`). Same leftover on
PWM mass-action / duplicate-field lists and File Manager
`cloud_base.ir_actions_server_ids`. Successor:

```python
# BEFORE — 500 when the param is stored empty
safe_eval(Config.get_str("odoo_password_manager.pw_custom_portal_fields", "[]"))
# AFTER
safe_eval(Config.get_str("odoo_password_manager.pw_custom_portal_fields", "[]") or "[]")
```

Checker `python-api` greps `safe_eval(...get_str` without `or`. 19.0 `get_param` of a
stored empty is also `""` — latent until settings are saved. Do not 20-only change
the inverse; fix the reader.
...
### `resource.calendar.tz` removed — timezone lives on the resource

19.0 `resource.calendar.tz` is gone at saas-19.4 (`addons/resource/models/resource_calendar.py`
has no `tz`; core `resource_data.xml` / `resource_demo.xml` no longer write it). Timezone is
`resource.resource.tz` / `resource.mixin.tz` (related, writable). Core `_work_intervals_batch`
takes `resources_per_tz`; a leftover `resource_calendar_id.tz` is `False` and `timezone(False)`
raises.

Observed `20_16` Gx.4: Layer-1 `demo_catalog.xml` died
`ValueError: Invalid field 'tz' in 'resource.calendar'` and Odoo installed the module
**without demo** (`load_demo` swallows the ParseError). Slot code then read
`resource_calendar_id.tz`. Successor: drop `tz` on the calendar; write `tz` on
`business.resource` (mixin); read `resource.tz`. Same attendance port: saas dropped
`resource.calendar.attendance.name` (required Char on 19.0); `day_period` is computed.
A 24/7 demo is one `hour_from=0` / `hour_to=24` row per weekday (saas onchange allows 24).

Do **not** add a bare `tz` to `field-lit` — `res.users` / `resource.resource` /
`resource.mixin` still have it. Checker greps `resource_calendar_id.tz`,
`timezone(self.tz)` (calendar method leftover), and `<field name="tz">`
inside a `resource.calendar` record.

```python
# BEFORE
tz = resource.resource_calendar_id.tz
# AFTER
tz = resource.tz or "UTC"
```

```xml
<!-- BEFORE -->
<record id="demo_calendar" model="resource.calendar">
    <field name="tz">Europe/Brussels</field>
</record>
<!-- AFTER: tz on the resource.mixin record, not the calendar -->
<record id="demo_resource" model="business.resource">
    <field name="tz">Europe/Brussels</field>
</record>
```
...
/ `object` / `token` / `title` / `size` / `mobileSize` / `service` /
`entries` / `card_href` / `card_records` / `_classes` / `card_active` /
`card_show_image` / `card_label` / `card_details_href` / `no_breadcrumbs` /
`additional_title` (not only the first child). Observed `20_14` Gx.8 token
share, `20_15` Gx.7 snippet palette (`10 - size` with `size is None`), and
`20_16` Gx.7 Appointments cards / portal sidebar / service price.
...
**Do not coerce `jstreeData || []`.** 19.0 `action_get_hierarchy` returns
`False` / `None` when the optional section is off (PWM `portal_vaults` /
types dummy; File Manager tags / shares). `[]` is truthy, so `ready` +
`canUpdate` still paints the header. Successor:
`this.treeData = Array.isArray(jstreeData) ? jstreeData.slice() : jstreeData`
and keep the 19.0 `t-if` (`treeData and (length or canUpdate)`). File
Manager `NodeJsTree` already keeps the sentinel. Observed `20_4` Gx.8
Portal Vaults header with the setting off. Checker `js-symbol` greps
`(jstreeData || [])`.
...
- **Bare `state.` in a `t-inherit` is not a `t-ref`.** OWL 3 compile scope is
  ...
  bare `t-att-*` / bare `t-on-click` idents / a whole-expression
  `t-if="projectUser"` (`20_4` Gx.8 systray) / a bare
  `t-props="viewProps"` (`20_4` Gx.8 PasswordLoginDialog).
- **`static props = { ...CharField.props }` is not the saas schema.**
  saas `CharField` is instance `props = props(charFieldProps)` and
  strips unknown keys. A leftover static spread never keeps `logIn`
  (`20_4` PasswordCopy regenerate on login). Checker `js-symbol`
  greps `...CharField.props`. Successor is `props = props({
  ...charFieldProps, extra: t.boolean().optional() })`.
- **`config.orderBy = []` then `sortBy` is not a new order.**
  `DynamicList.orderBy` is `config.orderBy`. saas `sortBy` toggles;
  a pre-written `asc=false` first term becomes `[]` / `defaultOrderBy`.
  PWM Sort by did nothing (`20_4` Gx.8). Successor `list.load({
  orderBy })`. Checker `js-symbol` greps `config.orderBy = []`.
- **`getRecordClasses()` is not the saas card class.** saas
  `KanbanRecord` extends `CardRenderer`; the article uses
  `this.getCardClasses()`. A leftover `getRecordClasses` never
  runs (`20_4` PWM / `20_14` File Manager checkbox). Successor
  `getCardClasses()`. Checker `js-symbol` greps `getRecordClasses(`.
  KPI already used `getCardClasses`.
- **`safe_eval(get_str(..., "[]"))` is not the default.** saas
  `get_str` ignores `default` when the key exists as `""`
  (`ir_config_parameter.py:124-129`). Settings inverse writes
  empty. `safe_eval("")` is `SyntaxError` (`20_4` Gx.8 portal
  vault after login). Successor `or "[]"`. Checker `python-api`.
- **`(jstreeData || [])` is not the 19.0 optional-section hide.**
  19.0 `action_get_hierarchy` returns `False` / `None` when the
  feature is off. Off-proxy copy that coerces to `[]` makes
  `ready` + `canUpdate` paint the header (`20_4` Portal Vaults
...
- **`hasclass()` is not `contains(@class)`.** OWL inherit XPath in this
  hub uses both. Indexing only `hasclass()` reported the PWM login
  inherit of `web.FormView.Buttons` clean while Save had already left
  that `t-name`. `owl-xpath` now indexes `contains(@class, …)` the same
  way (`20_4` Gx.8 login footer).
- **`record.data.bundle_id[0]` is not a named import.** `js-symbol` on
  exports missed it. saas `browse` then asserts `Invalid falsy real id`.
  Checker greps `.\w+_id[0]` (`20_4` PasswordCopy / kanban).
...
- *(2026-09-15, `20_14` Gx.8)* After jquery loaded, File Manager died on
  ...
  on `t-props`. Checker `owl-this` flags `t-props="getX()"` without
  `this.` **and** a bare ident `t-props="viewProps"` (`20_4`
  PasswordLoginDialog — View then has no `resModel`).
...
- **A bare `tz` is not `resource.calendar.tz`.** saas still has
  `res.users.tz` / `resource.resource.tz` / `resource.mixin.tz`. A
  `REMOVED_FIELD_LITERALS["tz"]` pass would flag every demo resource
  and the Appointments form. Bind to `resource_calendar_id.tz` and to
  `<field name="tz">` inside a `resource.calendar` record only.
  Observed `20_16` Gx.4.
...
- *(2026-09-16, `20_4` B1)* `pwm_jstree_container` still had
  ...
  `#{name}` interpolations.
- *(2026-09-17, `20_4` Gx.8)* Task Numbers systray search never
  rendered. `t-if="projectUser"` is a whole-expression bare ident —
  OWL 3 compile scope is `this`, so the input stayed hidden even
  though `onWillStart` set `this.projectUser` for Project users.
  Successor: `t-if="this.projectUser"`. Checker `owl-this` now
  flags `t-if` / `t-elif` whose value is only `ident` / `!ident`
  (`true` / `false` / `this.` stay quiet). Retro-scan of
  `20_2` / `20_c` / `20_14` / `20_5` / `20_6` / `20_9` / `20_10`
  / `20_port`: only this file.
- *(2026-09-17, `20_4` Gx.8)* Password Manager first-clicks died
  on two leftover 19.0 OWL/JS shapes the checker had reported
  clean. Bundle card:
  `archInfo.openAction.action` — saas moved `openAction` onto the
  `KanbanRecord` prop (`kanban_renderer.xml:53`; record `:237`).
  All passwords login dialog:
  `<View t-props="viewProps"/>` — OWL 3 does not bind the bare
  ident, so `View.setup` raised `View props should have a
  "resModel" key` even though JS set `this.viewProps.resModel`.
  Successor: `this.props.openAction` (keep `buttonContext`) and
  `t-props="this.viewProps"` / `modalRef="this.modalRef"` (saas
  `web.FormViewDialog`). Checker `js-symbol` greps
  `archInfo.openAction`; `owl-this` now flags a bare
  `t-props="ident"` / `modalRef="ident"`. Retro-scan of
  `20_2` / `20_c` / `20_14` / `20_5` / `20_6` / `20_9` / `20_10`
  / `20_port`: only these two files.
- *(2026-09-17, `20_4` Gx.8)* Password Manager login still showed
  Save / Discard, Enter did not log in, copy / regenerate stayed
  visible, and eye / copy died `Invalid falsy real id` on
  `password.bundle`. Three leftover 19.0 shapes: (1) inherit of
  `web.FormView.Buttons` — saas dialogs use
  `buttonDialogTemplate` / `web.FormView.DialogButtons`;
  (2) `this.props.logIn` dropped by saas `CharField` `props()` so
  the login widget treated the field as a normal password;
  (3) `bundle_id[0]` — many2one is `{id, display_name}`, so the
  security RPC browsed `[undefined]`. Successor: inherit
  `DialogButtons` + pass `buttonDialogTemplate`; read `log_in`
  from `activeFields.attrs`; `value.id` (and filter falsy ids
  before `orm.call`). Checker `owl-xpath` now indexes
  `contains(@class)`; `js-symbol` greps `.\w+_id[0]`. Retro-scan
  of `20_2` / `20_c` / `20_14` / `20_5` / `20_6` / `20_9` /
  `20_10` / `20_port`: only these PWM files.
- *(2026-09-17, `20_4` Gx.8)* After DialogButtons, login still showed
  regenerate and Enter did not log in. `static props = {
  ...CharField.props, logIn }` does not keep extras: saas
  `CharField` uses instance `props = props(charFieldProps)` and
  strips unknown keys. `activeFields.attrs.log_in` was also empty
  at runtime. Successor: `props = props({ ...charFieldProps,
  logIn: t.boolean().optional() })` (enterprise
  `AccountAuditClickableCharField`). Checker `js-symbol` greps
  `...CharField.props`. Retro-scan of `20_2` / `20_c` / `20_14` /
  `20_5` / `20_6` / `20_9` / `20_10` / `20_port`: only this file.
- *(2026-09-17, `20_4` Gx.8)* All Passwords Sort by did nothing.
  PWM wrote `config.orderBy` (`asc=false`) then `sortBy(fieldName)`.
  `orderBy` is that same array; saas `sortBy` toggles a desc first
  term to `[]` and reloads `defaultOrderBy` (name). Successor:
  `kanbanList.load({ orderBy })`. File Manager already `_load`s.
  Checker `js-symbol` greps `config.orderBy = []`. Retro-scan of
  `20_2` / `20_c` / `20_14` / `20_5` / `20_6` / `20_9` / `20_10` /
  `20_port`: no leftover. Same walk: Portal Vaults empty because
  `action_get_hierarchy` needs `group_portal_password_vaults`;
  Layer 1 rows existed; Gx.5 wiring never implied the group onto
  `base.group_user` / `base.group_portal`. Stamp that in
  `_demo_pwm_stamp_settings`.
- *(2026-09-17, `20_4` Gx.9 review)* After Sort by used `this.asc`,
  jstree `onUpdateSearch` still passed `asc: !this.asc` (the 19.0
  `sortBy` toggle convention). Checking a folder then reloaded the
  opposite order. Successor: `asc: this.asc` on
  `toggleJSTreeDomain` too. Checker `js-symbol` greps
  `asc: !this.asc`. Retro-scan: only this PWM file.
- *(2026-09-17, `20_4` Gx.8)* Selected PWM cards had no checked box.
  saas `KanbanRecord` is a `CardRenderer`; the article class is
  `this.getCardClasses()` (`kanban_record.js:120`). Leftover
  `getRecordClasses()` never ran, so `jstr-kanban-selected` never
  landed and CSS never swapped `.fa-square-o` → `.fa-check-square-o`.
  Same leftover on File Manager `20_14` / `20_port`. KPI already
  used `getCardClasses`. Successor: override `getCardClasses()`.
  Checker `js-symbol` greps `getRecordClasses(`. Same walk: demo
  still saw Sales `Google Adsense` after Access Groups
  Administrator because `password_key_read_rule` excepts the
  bundle creator and Full/Admin; Layer 2 had stamped 0user as
  both. Restriction itself is green (a non-creator readonly user
  is denied). Successor: `_demo_pwm_stamp_sales_access_story`
  (Sales 0user readonly, Sales `create_uid` admin, stamp
  `demo_key_adsense_sales` with `base.group_system`).
- *(2026-09-17, `20_4` Gx.8)* Portal vault login 500 after
  `action_check_entered_password` 200. GET of
  `/my/password_vaults/<slug>` hit `_get_custom_fields` →
  `safe_eval(Config.get_str("odoo_password_manager.pw_custom_portal_fields", "[]"))`
  → `SyntaxError: invalid syntax`. Stored ICP is `""` (settings
  inverse when the m2m is empty). saas `get_str` default is unused
  when the key exists (`ir_config_parameter.py:124-129`). Same
  leftover on PWM mass-action / duplicate lists and File Manager
  `cloud_base.ir_actions_server_ids` (`20_14` / `20_port`).
  Successor: `safe_eval(get_str(...) or "[]")`. Checker
  `python-api`. Retro-scan of `20_2` / `20_c` / `20_5` / `20_6` /
  `20_9` / `20_10`: no leftover.
- *(2026-09-17, `20_4` Gx.8)* Portal Vaults header stayed on All
  Passwords after Settings "Portal vaults" was unchecked. 19.0
  `action_get_hierarchy("portal_vaults")` returns `False` without
  the group; `t-if="state.treeData and …"` hid the section. The
  off-proxy port did `jstreeData || []` and
  `t-if="ready and (length or canUpdate)"` — `[]` is truthy and
  admin `canUpdate` paints the header. Same leftover would show
  Types when `odoo_password_manager_custom_types` is absent
  (`_return_types_nodes` is `None`). File Manager
  `NodeJsTree` already keeps `None` (`clouds_node` tags/shares).
  Successor: copy only arrays; restore the 19.0 `t-if`. Checker
...
- *(2026-09-18, `20_11` Gx.3)* `owl-this` flagged
  `t-out="groupColumn"` / `t-if="reportWarning"` after
  `t-foreach` / `t-set`. Those are compile-scope locals —
  core saas still writes `t-as="record"` then `record.name`.
  Prefixing `this.` would hide the column. Successor: keep
  the alias; checker skips `t-as` / `t-set` names. Prefix
  `state.` → `this.state.` and FormView xpath
  `props.Renderer` → `this.props.Renderer` (19.0
  `form_controller.xml` still had the bare name). saas
  Chart.js is v4.5.0; leftover `options.xAxes` is Chart 2.
  Successor: destroy/recreate with `scales.x`. Retro-scan
  of already-ported groups: no leftover `xAxes` / bare
  `t-as` false positive after the skip.
- *(2026-09-18, `20_11` Gx.8)* Sales Trends Refresh / Save /
  Export died `this.props.refreshReport is not a function`
  (same for `reloadReports` on Save). View still copies
  extra keys into `viewProps`; saas `FormController`
  `props = props(formControllerProps)` then strips them.
  Leftover `static props = { ...FormController.props,
  refreshReport }` is the CharField hole. Successor:
  `props = props({ ...formControllerProps, extra })`
  (`mrp_workorder` WorkorderFormController). Checker
...
- *(2026-09-19, `20_15` Gx.8 walk)* Website Customize on KnowSystem /
  ...
  `website-options`.
- *(2026-09-19, `20_16` Gx.4)* `resource.calendar.tz` is gone. Layer-1
  `demo_catalog.xml` died `Invalid field 'tz' in 'resource.calendar'`;
  saas `load_demo` then installed the module without demo, so Gx.5
  counts stayed 0. Successor: drop calendar `tz`; write/read
  `resource.mixin.tz`. Same file: saas attendance has no `name`
  (19.0 required Char) and `day_period` is computed — one
  `hour_from=0` / `hour_to=24` row per weekday. Checker `field-lit`
  greps `resource_calendar_id.tz` and a `tz` field on a
  `resource.calendar` record, not a bare `tz` (users / mixin keep it).
- *(2026-09-19, `20_16` Gx.5)* Stamp died
  `TypeError: product.product.image_1920: use BinaryValue instead of
  bytes` at `_demo_ba_stamp_car_rent_variants`. saas
  `convert_to_cache` accepts `BinaryValue` or a base64 **str**, not
  `bytes` (`fields_binary.py`). 19.0 `base64.b64encode(fh.read())` is
  bytes. Successor `BinaryBytes(fh.read())`. Same leftover on
  `_demo_cf_put_file`. Checker `python-api` greps `base64.b64encode(`.
- *(2026-09-19, `20_16` Gx.5)* After calendar XML `tz` was dropped,
  stamp still died `AttributeError: 'resource.calendar' object has no
  attribute 'tz'` at `_ba_attendance_intervals`. Leftover
  `timezone(self.tz)`. Successor: `resource.tz` (the
  `resource.resource` passed in) or context/user/`UTC`. Checker
  `field-lit` greps `timezone(self.tz)`.
- *(2026-09-19, `20_16` Gx.7)* Inner `t-set` of `service` /
  `entries` / `card_href` / `card_records` / `_classes` /
  `no_breadcrumbs` / `additional_title` never reaches the
  callee. `/appointments/services/1` died `KeyError: 'service'`;
  type cards had empty `<a class="stretched-link">`; portal
  detail missed `#ba_functional_buttons`; breadcrumbs stayed
  on. Successor: `t-call` attributes (sibling `t-set` when the
  slot also reads the name). Checker `qweb-tcall` now matches
  those keys.
- *(2026-09-19, `20_16` Gx.7)* `_ensure_ba_frontend_translations`
  called `update_field_translations` on `website.ba_step*` in
  `lang=en_US`. saas `_update_field_translations` SQL-merges
  then `self[field_name] = self[field_name]`. That write-back
  of the current-lang string drops other JSON keys, so
  `ru_RU` stayed `Choose resource type`. `prefetch_langs=True`
  then dies `StoredTranslations` on the jsonb string
  (`ValueError: dictionary update sequence element #0`).
  Successor: `record.write({fname: {"en_US": current, **mapping}})`
  then `flush_recordset`. Char `write` of a dict is cache-only
  (`get_column_update` emits jsonb on flush); a one-lang dict
  takes the string path. `cr.commit()` is forbidden in HttpCase.
  Assert the SQL jsonb, not only `with_context(lang=ru_RU)`.
  View-term `arch_db` (`{lang: {src: text}}`) already works —
  do not change that path. Flush is **not** HTTP proof: after
  this write, TransactionCase SQL+ORM was green and
  `/ru/appointments/1` still rendered `Choose resource type`
  on `website.ba_step1` (page chrome was already ru). Successor
  for QWeb: `website._ba_frontend_text(fname)` /
  `_ba_step_labels()` using `request.lang` then the packs when
  the stored visitor-lang value equals the English source.
  Checker `qweb-tcall` greps leftover `website.ba_stepN`.
- *(2026-09-19, `20_16` Gx.7)* After the Char write+flush landed,
  `test_ru_stepper_and_signin_render` still failed
  `Выберите тип ресурса` not in `/ru/appointments/1`. HTML
  was `lang="ru-RU"` with Russian menu/cards; stepper,
  `<title>`, and searchbar brand stayed English. Live SQL
  on the review DB was still `{'en_US': 'Choose resource
  type'}` only; HttpCase after `_ensure`+flush still
  rendered the English Char. Successor: do not trust saas
  Char translate on website records during frontend QWeb.
  `website._ba_frontend_text` / `_ba_step_labels` read
  `request.lang` then the packs when stored lang equals
  the English source. Keep `_ensure` for settings jsonb.
  `cr.commit()` stays forbidden. Do not claim flush-alone
  is HTTP proof.
